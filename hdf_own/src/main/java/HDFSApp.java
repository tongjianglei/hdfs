...
private enum Type {
    ...
    public static void hdfsDataSplit() {
        // 获取FileSystem实例
        String fileUri = "hdfs://11.160.74.200:9000/hivewarehouse/d2c_test_2";
        String splitDelimiter = "\001";
        ...
        String specifiedFileType = conf.getNecessaryValue(com.alibaba.datax.plugin.reader.hdfsreader.Key.FILETYPE, com.alibaba.datax.plugin.reader.hdfsreader.HdfsReaderErrorCode.REQUIRED_VALUE);
        com.alibaba.datax.plugin.reader.hdfsreader.DFSUtil dfsUtil = new com.alibaba.datax.plugin.reader.hdfsreader.DFSUtil(conf);
        ...
        if(specifiedFileType.equalsIgnoreCase(com.alibaba.datax.plugin.reader.hdfsreader.Constant.TEXT)){
            ...
            HashSet<String> hash =  dfsUtil.getAllFiles(path, com.alibaba.datax.plugin.reader.hdfsreader.Constant.RC);
            List<String> sourceFiles = new ArrayList<>(hash);
            for(String sourceFile : sourceFiles){
                rcFileStartRead(sourceFile, conf);
            }
        }
    }
    public static List<List<Object>> readFromStream(InputStream inputStream, String context,
...
//                    Constant.DEFAULT_ENCODING));
        ...
        try {
            ...
            return doReadFromStream(reader, context,
                    readerSliceConfig);
...
//                LOG.error(message);
                ...
                throw DataXException.asDataXException(com.alibaba.datax.plugin.reader.hdfsreader.HdfsReaderErrorCode.READ_FILE_ERROR, message);
            }
        } else {
            String message = String.format("请确认您所读取的列配置正确！columnIndexMax 小于0,column:%s", JSON.toJSONString(column));
            throw DataXException.asDataXException(com.alibaba.datax.plugin.reader.hdfsreader.HdfsReaderErrorCode.BAD_CONFIG_VALUE, message);
        }
        return res;
    }
    public static List<List<Object>> rcFileStartRead(String sourceRcFilePath, Configuration readerSliceConfig) {
//        LOG.info(String.format("Start Read rcfile [%s].", sourceRcFilePath));
        ...
        try {
            ...
            while (recordReader.next(key, value)) {
                ...
                res.add(new ArrayList<>(transportOneRecord(column, sourceLine, nullFormat)));
            }
...
//            LOG.error(message);
            System.out.println(message);
            throw DataXException.asDataXException(com.alibaba.datax.plugin.reader.hdfsreader.HdfsReaderErrorCode.READ_RCFILE_ERROR, message, e);
        } finally {
            try {
...
//                LOG.error(message);
                System.out.println(message);
                throw DataXException.asDataXException(com.alibaba.datax.plugin.reader.hdfsreader.HdfsReaderErrorCode.CONFIG_INVALID_EXCEPTION, message);
